  ...
  <div class="admin-faq-manager">
    ...
    <Row :style="{marginBottom: '10px'}">
      ...
      <Col span="6" :style="{textAlign: 'left'}">
        <Button type="success" @click="newTag">新增</Button>
      </Col>
    </Row>
    <Table border :columns="tableColumns" :data="tableData">
      <template slot-scope="{ row, index }" slot="action">
        <div>
          ...
          <Button type="success" size="small" @click="onEdit(row)">编辑</Button>
        </div>
      </template>
    </Table>
    <Page
...
export default {
  ...
  methods: {
    ...
    auditState (id, pass) {
      this.$Loading.start()
      this.$http.post('/tag/audit-state', { id, is: pass }).then(res => {
        if (res.code !== 0) {
          this.$Message.error(res.message)
          this.$Loading.error()
          return
    ...
    updateTag () {
      this.$refs.newForm.validate((valid) => {
        ...
        this.$Loading.start()
        this.$http.post('/tag/update', this.newForm).then(res => {
          if (res.code !== 0) {
            this.$Loading.error()
            this.$Message.error(res.message)
            return
    ...
    createOk () {
      this.$refs.newForm.validate((valid) => {
        ...
        this.$Loading.start()
        this.$http.post('/tag/create', this.newForm).then(res => {
          if (res.code !== 0) {
            this.$Loading.error()
            this.$Message.error(res.message)
            return
